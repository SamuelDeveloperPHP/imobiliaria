...
if($nivel_usuario == 'Corretor'){
	$query = $pdo->query("SELECT * FROM imoveis where status = 'Para Aluguél' corretor = '$id_usuario' ORDER BY id desc");
}else{
	$query = $pdo->query("SELECT * FROM imoveis where status = 'Para Aluguél' ORDER BY id desc");
}
...
//retirar aspas do texto do obs
		...
		if($comissao_corretor != "0" || $comissao_imob != "0"){
			...
			$comissao_corretor_RS = ($comissao_aluguel_corretor / 100) * $valor;
		}
		...
		$query2 = $pdo->query("SELECT * FROM vistorias where imovel = '$id'");
		$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
		if(@count($res2) > 0){
		...
		<a href="#" onclick="alugar('{$id}', '{$titulo}', '{$comissao_imob_RS}', '{$comissao_corretor_RS}', '{$valor}', '{$valorF}')" title="Alugar Imóvel"><i class="fa fa-money " style="color:green; margin-left:3px"></i></a>

		<a href="#" onclick="vistoria('{$id}', '{$titulo}', '{$texto_vistoria}')" title="Laudo de Vistoria"><i class="fa fa-file-pdf-o " style="color:red; margin-left:3px"></i></a>
...
function alugar(id, nome, comissao_imob, comissao_corretor, valor, valorf){
    $('#id-vender').val(id);    
