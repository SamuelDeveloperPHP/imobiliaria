...
if($nivel_usuario == 'Corretor'){
	$query = $pdo->query("SELECT * FROM imoveis where status = 'Alugado' corretor = '$id_usuario' ORDER BY id desc");
}else{
	$query = $pdo->query("SELECT * FROM imoveis where status = 'Alugado' ORDER BY id desc");
}
...
//retirar aspas do texto do obs
		...
		//verificar a locação associada ao imóvel
		$query2 = $pdo->query("SELECT * FROM alugueis where imovel = '$id'");
		$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
		if(@count($res2) > 0){
			$id_aluguel = $res2[0]['id'];
		}
		...
		<a href="rel/contrato_locacao_class.php?id={$id_aluguel}" target="_blank" title="Gerar Contrato"><i class="fa fa-file-archive-o " style="color:blue; margin-left:3px"></i></a>
	...
	echo 'Não possui nenhum Imóvel Alugado!';
}
