...
if($nivel_usuario == 'Corretor'){
	$query = $pdo->query("SELECT * FROM imoveis where status = 'Vendido' corretor = '$id_usuario' ORDER BY id desc");
}else{
	$query = $pdo->query("SELECT * FROM imoveis where status = 'Vendido' ORDER BY id desc");
}
...
//retirar aspas do texto do obs
		...
		//verificar a venda associada ao imóvel
		$query2 = $pdo->query("SELECT * FROM vendas where imovel = '$id'");
		$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
		if(@count($res2) > 0){
			$id_venda = $res2[0]['id'];
		}else{
		...
		<a href="rel/contrato_venda_class.php?id={$id_venda}" target="_blank" title="Gerar Contrato"><i class="fa fa-file-archive-o " style="color:blue; margin-left:3px"></i></a>
	...
	echo 'Não possui nenhum Imóvel Vendido!';
}
